...
const DroneDetails = ({ attitude, ports, connectedPort, onScan, onConnect }) => {
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div style={{ background: 'white', padding: '20px', borderRadius: '12px', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '15px', border: `1px solid ${THEME.border}` }}>
        <button onClick={onScan} style={{ padding: '10px 20px', background: '#F0F0F0', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>Refresh Ports</button>
        {ports.length === 0 && <span style={{color: THEME.textLight}}>No Devices Found</span>}
        {ports.map((p, i) => (
          <button key={i} onClick={() => onConnect(p.path)} 
